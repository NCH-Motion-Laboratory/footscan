...
for step in steps_before_left:
    points = np.column_stack((step.cop_x, step.cop_y, T_SCALE * np.arange(step.data.shape[2]) * np.mean((step.dx, step.dy))))
    tube = pv.Spline(points, 1000).tube(radius=0.5)
    left_tubes_before.append(tube)

for step in steps_before_right:
    points = np.column_stack((step.cop_x + maxx, step.cop_y, T_SCALE * np.arange(step.data.shape[2]) * np.mean((step.dx, step.dy))))
    tube = pv.Spline(points, 1000).tube(radius=0.5)
